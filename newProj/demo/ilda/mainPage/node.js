...
function createAddWork(elem) {
    ...
    xmlHttpRequestServices.addEventListener("readystatechange", () => {
        if (xmlHttpRequestServices.readyState === 4 && (xmlHttpRequestServices.status === 200)) {
            ...
            serviceField.size = table.id.length;
            for (var i = 0; i < table.id.length; i++) {
    ...
    var serviceId = localStorage.getItem('serviceInfo');
    var xmlHttpRequestService = new XMLHttpRequest();
    xmlHttpRequestService.open('GET', 'http://localhost:8080/bt/services/' + serviceId, true);
    xmlHttpRequestService.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('token'));
    xmlHttpRequestService.send()
    var countMessage = 0;
    xmlHttpRequestService.addEventListener("readystatechange", () => {
        if (xmlHttpRequestService.readyState === 4 && xmlHttpRequestService.status === 200) {
            var serviceText = xmlHttpRequestService.responseText;
            serviceWork = JSON.stringify(serviceText);
        } else if (xmlHttpRequestService.status === 404 && countMessage < 1) {
            countMessage++;
            alert("There is no service with id " + serviceId)
        }
    });
    var carId = localStorage.getItem('carInfo');
    var xmlHttpRequestCar = new XMLHttpRequest();
    xmlHttpRequestCar.open('GET', 'http://localhost:8080/bt/cars/' + carId, true);
    xmlHttpRequestCar.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('token'));
    xmlHttpRequestCar.send()
    countMessage = 0;
    xmlHttpRequestCar.addEventListener("readystatechange", () => {
        if (xmlHttpRequestCar.readyState === 4 && xmlHttpRequestCar.status === 200) {
            var carText = xmlHttpRequestService.responseText;
            carWork = JSON.stringify(carText);
        } else if (xmlHttpRequestCar.status === 404 && countMessage < 1) {
            countMessage++;
            alert("There is no car with id " + carId)
        }
    });

    var masterId = localStorage.getItem('masterInfo');
    var xmlHttpRequestMaster = new XMLHttpRequest();
    xmlHttpRequestMaster.open('GET', 'http://localhost:8080/bt/masters/' + masterId, true);
    xmlHttpRequestMaster.setRequestHeader('Authorization', 'Bearer ' + localStorage.getItem('token'));
    xmlHttpRequestMaster.send()
    countMessage = 0;
    xmlHttpRequestMaster.addEventListener("readystatechange", () => {
        if (xmlHttpRequestMaster.readyState === 4 && xmlHttpRequestMaster.status === 200) {
            var masterText = xmlHttpRequestMaster.responseText;
            masterWork = JSON.stringify(masterText);
        } else if (xmlHttpRequestMaster.status === 404 && countMessage < 1) {
            countMessage++;
            alert("There is no car with id " + masterId)
        }
    });
    newButton.onclick = function () {
        let json = JSON.stringify({
            "date": toString(dateField.value),
            "cars": {
                carWork
            },
            "masters" : {
